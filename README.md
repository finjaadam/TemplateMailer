...
The TemplateMailer package requires SymfonyMailer to send out e-mails. Please check the SymfonyMailers package's
configuration options (https://github.com/neos/symfonymailer) in order to configure SMTP credentials.
...
**_IMPORTANT:_** You _must_ create a .txt and an .html file with the same name for each template-based mail you want to send.
...
If you pass an array to the `sendTemplateEmail()` method, we'll pass it right through to SymfonyMailer so you can
